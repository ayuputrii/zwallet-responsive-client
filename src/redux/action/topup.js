...
export const chargeTopup20k = (token) => async dispatch => {
    const res = await axios.post(`${URI}/topup/charge`, { amount: 20000 }, {
        headers: {
            Authorization: `Bearer ${token}`
        }
    })
...
export const chargeTopup50k = (token) => async dispatch => {
    const res = await axios.post(`${URI}/topup/charge`, { amount: 50000 }, {
        headers: {
            Authorization: `Bearer ${token}`
        }
    })
...
export const chargeTopup100k = (token) => async dispatch => {
    const res = await axios.post(`${URI}/topup/charge`, { amount: 100000 }, {
        headers: {
            Authorization: `Bearer ${token}`
        }
    })
